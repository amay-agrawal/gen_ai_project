...
export default function Campaigns() {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      console.error(err);
      alert(err.message || 'Failed to parse intent.');
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      if (parsedIntent.intent.intent === 'bulk_outreach') {
        ...
        const contactId = parsedIntent.resolvedContacts[0]?.contactId;
        if (!contactId) {
          alert(`No contact found in CRM for "${parsedIntent.resolvedContacts[0]?.company || 'recipient'}". Please add them to your CRM first.`);
          return;
        }
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-4xl pb-16">
      ...
      <Card className="glass-panel border-0 shadow-2xl relative overflow-hidden">
        <div className="absolute -inset-1 bg-gradient-to-r from-primary to-accent opacity-20 blur-xl rounded-3xl pointer-events-none" />
        <CardHeader>
